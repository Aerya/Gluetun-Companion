...
def fetch_gluetun_servers(gluetun_host: str, gluetun_api_port: int) -> list[dict] | None:
    ...
    NOTE: This endpoint does not exist in current versions of Gluetun (not part
    of the documented API). It is kept for forward-compatibility in case it is
    added in a future release.  The caller should fall back to
    ``fetch_gluetun_settings_names`` when this returns None.

    Returns:
      - list[dict] on success (may be empty if provider has no servers)
      - None if the API is unreachable OR the endpoint returns 404
    """
    url = f'http://{gluetun_host}:{gluetun_api_port}/v1/servers'
    try:
        resp = requests.get(url, timeout=8)
        if resp.status_code == 404:
            logger.debug('fetch_gluetun_servers: endpoint not found (404) at %s', url)
            return None
        resp.raise_for_status()
...
def fetch_gluetun_settings_names(gluetun_host: str, gluetun_api_port: int) -> list[dict] | None:
    """
    Fallback discovery via GET /v1/vpn/settings (always available in Gluetun).

    Extracts the currently configured server names / hostnames from the
    provider.server_selection section and returns them as minimal server dicts
    compatible with the api_gluetun_servers JSON response format.

    Returns:
      - list[dict] with keys: name, hostname, country, city, region
        (country/city/region are empty strings — not available from settings)
      - None on failure (API unreachable or unexpected response format)
    """
    url = f'http://{gluetun_host}:{gluetun_api_port}/v1/vpn/settings'
    try:
        resp = requests.get(url, timeout=8)
        resp.raise_for_status()
        data = resp.json()
        ...
        server_sel = (
            data.get('provider', {}).get('server_selection', {})
            or {}
        )
        names     = [n for n in (server_sel.get('names')     or []) if n]
        hostnames = [h for h in (server_sel.get('hostnames') or []) if h]

        result: list[dict] = []
        for n in names:
            result.append({'name': n, 'hostname': '', 'country': '', 'city': '', 'region': ''})
        for h in hostnames:
            if h not in names:           # avoid duplicates if both are set
                result.append({'name': '', 'hostname': h, 'country': '', 'city': '', 'region': ''})

        logger.debug(
            'fetch_gluetun_settings_names: found %d server(s) via /v1/vpn/settings', len(result)
        )
        return result
    except requests.exceptions.ConnectionError as exc:
        logger.warning('fetch_gluetun_settings_names: unreachable at %s — %s', url, exc)
        return None
    except Exception as exc:
        logger.warning('fetch_gluetun_settings_names: %s', exc)
        return None
