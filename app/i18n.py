...
TRANSLATIONS: dict[str, dict[str, str]] = {
    'fr': {
        ...
        'srv_profile_info':           'Indicatif uniquement — sans profil sélectionné dans Paramètres (Bascule auto) ou bascule manuelle vers le serveur recommandé, aucun changement de serveur n\'a lieu.',
        'srv_profile_mono':           'Tous les profils recommandent le même serveur : les métriques jitter, perte et DNS ne sont disponibles qu\'en mode sidecar. En mode proxy, seuls le débit et la latence différencient les serveurs.',
        ...
        'set_gluetun_api_port_label':     'Port API Gluetun',
        'set_gluetun_api_port_hint':      'Port de l\'API de contrôle Gluetun (<code>HTTP_CONTROL_SERVER_PORT</code>, défaut : 8000). Doit être exposé dans le <code>docker-compose.yml</code> pour que la découverte de serveurs fonctionne.',
        'srv_discover':                   'Découvrir',
        'srv_discover_tooltip':           'Interroge l\'API Gluetun pour lister les serveurs VPN disponibles et les importer dans Companion en un clic. Nécessite que le port de l\'API Gluetun soit exposé dans votre docker-compose.',
        'srv_discover_from_settings':     'Données issues de /v1/vpn/settings (serveurs actuellement configurés dans Gluetun). L\'endpoint /v1/servers n\'est pas disponible dans cette version de Gluetun.',
        'srv_discover_title':             'Serveurs disponibles dans Gluetun',
        'srv_discover_filter_hint':       'Filtre Gluetun actif :',
        'srv_discover_all':               'Tous les serveurs du provider',
        'srv_discover_select_all':        'Tout sélectionner',
        'srv_discover_deselect_all':      'Tout désélectionner',
        'srv_discover_import_btn':        'Importer la sélection',
        'srv_discover_already':           'déjà ajouté',
        'srv_discover_empty':             'Aucun serveur trouvé pour ce filtre.',
        'srv_discover_api_error':         'Impossible de joindre l\'API Gluetun. Exposez le port dans votre docker-compose.yml :',
        'srv_discover_tried':             'URLs tentées',
        'srv_discover_loading':           'Chargement…',
        'srv_discover_imported':          'serveurs importés',
    ...
    'en': {
        ...
        'srv_profile_info':           'Display only — without a profile selected in Settings (Auto-switch) or a manual switch to the recommended server, no server change takes place.',
        'srv_profile_mono':           'All profiles recommend the same server: jitter, packet-loss and DNS metrics are only available in sidecar mode. In proxy mode, only throughput and latency differentiate servers.',
        'set_api_title':           'REST API',
        ...
        'set_gluetun_api_port_label':     'Gluetun API port',
        'set_gluetun_api_port_hint':      'Gluetun control API port (<code>HTTP_CONTROL_SERVER_PORT</code>, default: 8000). Must be published in <code>docker-compose.yml</code> for server discovery to work.',
        'srv_discover':                   'Discover',
        'srv_discover_tooltip':           'Queries the Gluetun API to list available VPN servers and import them into Companion in one click. Requires the Gluetun API port to be exposed in your docker-compose.',
        'srv_discover_from_settings':     'Data from /v1/vpn/settings (servers currently configured in Gluetun). The /v1/servers endpoint is not available in this version of Gluetun.',
        'srv_discover_title':             'Available servers in Gluetun',
        'srv_discover_filter_hint':       'Active Gluetun filter:',
        'srv_discover_all':               'All provider servers',
        'srv_discover_select_all':        'Select all',
        'srv_discover_deselect_all':      'Deselect all',
        'srv_discover_import_btn':        'Import selection',
        'srv_discover_already':           'already added',
        'srv_discover_empty':             'No servers found for this filter.',
        'srv_discover_api_error':         'Cannot reach Gluetun API. Expose the port in your docker-compose.yml:',
        'srv_discover_tried':             'Tried URLs',
        'srv_discover_loading':           'Loading…',
        'srv_discover_imported':          'servers imported',
